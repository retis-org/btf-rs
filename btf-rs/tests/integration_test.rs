...
fn resolve_anon_name(btf: Btf) {
    ...
    let types = btf.resolve_types_by_name(ANON_TYPE_NAME).unwrap();
    ...
    let ids = btf.resolve_ids_by_name(ANON_TYPE_NAME).unwrap();
    assert_ne!(ids.len(), 0)
}
...
fn resolve_anon_regex(btf: Btf) {
    let re = regex::Regex::new(&format!("{ANON_TYPE_NAME}")).unwrap();
    ...
    // Try another anonymous name lookup variant.
    let re = regex::Regex::new(&format!(r"^{}$", regex::escape(ANON_TYPE_NAME))).unwrap();
    let ids = btf.resolve_ids_by_regex(&re).unwrap();
    assert_ne!(ids.len(), 0);
}
