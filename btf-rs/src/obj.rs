...
impl BtfObj {
    ...
    pub(super) fn from_mmap(mmap: Mmap, base: Option<Arc<BtfObj>>) -> Result<Self> {
        Ok(Self(Box::new(MmapBtfObj::from_mmap(mmap, base)?)))
    }
    ...
    pub(super) fn from_reader<R: Seek + BufRead>(
        ...
        Ok(Self(Box::new(CachedBtfObj::from_reader(reader, base)?)))
    }
...
/// (types, names, etc).
pub(crate) trait BtfBackend {
    /// Access the BTF header as a reference.
    fn header_ref(&self) -> &cbtf::btf_header;
...
impl CachedBtfObj {
    fn from_reader<R: Seek + BufRead>(reader: &mut R, base: Option<Arc<BtfObj>>) -> Result<Self> {
        ...
        let (mut id, start_str_off) = match base {
            None => (1, 0),
            Some(ref base) => (base.types() as u32, base.header_ref().str_len),
        };
        ...
        while reader.stream_position()? < end_type_section {
            let r#type = Type::from_reader(reader, &endianness)?;
            let name_off: u32 = match r#type.as_btf_type() {
                Some(bt) => bt.get_name_offset().unwrap_or(0),
                _ => 0,
            };
...
impl MmapBtfObj {
    fn from_mmap(mmap: Mmap, base: Option<Arc<BtfObj>>) -> Result<Self> {
        ...
        let (str_offset, type_offset) = match base {
            Some(base) => (base.header_ref().str_len, base.types() as u32),
            None => (0, 0),
        };
        ...
        F: FnMut(u32, &[u8]) -> Result<()>,
    {
        ...
        for (id, offset) in self.type_offsets.iter().enumerate() {
            let name_off = cbtf::btf_get_name_offset(&mmap[*offset..], &self.endianness)?;

            if name_off > 0 && name_off < self.header.str_len as usize {
                let start = self.header.hdr_len as usize + self.header.str_off as usize + name_off;

                f(id as u32 + self.type_offset, &mmap[start..])?;
            }
        }
...
impl BtfBackend for MmapBtfObj {
    ...
    fn resolve_type_by_id(&self, id: u32) -> Result<Option<Type>> {
        ...
        Ok(match self.type_offsets.get(id - 1) {
            Some(offset) => Some(Type::from_bytes(&self.mmap[*offset..], &self.endianness)?),
            None => None,
        })
    }
