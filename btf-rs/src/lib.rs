...
//!
//! [`Backend::Mmap`] is only supported for base BTF files and is used by
//! default there. [`Backend::Cache`] one is used for split BTF files and as a
//! fallback if a file can't be mmaped. This can however be explicitly
//! controlled using the dedicated constructor (see `from_*_with_backend`
//! helpers). All BTF data read from bytes use [`Backend::Cache`] (this includes
//! all `elf` feature helpers).
//!
...
//! - [`btf`] provides our representation of the BTF data in Rust and a unified
//!   API on top of the BTF metadata representations. Additional logic is
//!   allowed, e.g. the [`Btf`] object handles split files and anonymous types.
