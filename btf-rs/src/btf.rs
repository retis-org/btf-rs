...
impl Type {
    ...
    pub(crate) fn from_bytes(buf: &[u8], endianness: &cbtf::Endianness) -> Result<Self> {
        Self::from_reader(&mut Cursor::new(buf), endianness)
    }
...
impl Enum64Member {
    pub(crate) fn from_bytes(buf: &[u8], endianness: &cbtf::Endianness) -> Result<Enum64Member> {
        Ok(Enum64Member {
