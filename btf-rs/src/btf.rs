...
impl Btf {
    ...
    /// for later use. By default the `Backend::Mmap` is used, if supported,
    /// otherwise `Backend::Cache` is used.
    ///
    /// Trying to open split BTF files using this function will fail. For split
    /// BTF files use `Btf::from_split_file`.
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        Self::from_file_with_backend(&path, Backend::Mmap)
            .or_else(|_| Self::from_file_with_backend(&path, Backend::Cache))
    }
    ...
    pub(crate) fn resolve_split_ids_by_name(&self, name: &str) -> Result<Vec<u32>> {
        self.obj.resolve_ids_by_name(handle_anon_name!(name))
    }
